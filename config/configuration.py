...
THERMOSTAT_PORT: typing.Final[str] = (
    "COM3"  # Номер COM порта, присвоенный преобразователю USB>RS485
)
...
DATA_MAP: typing.Final[dict] = {
    "TEMP_MAX_LEVEL": 400,  # Максимальная температура (умножено на 10) Диапазон: -400...600 ºС, По умолчанию 400
    "TEMP_MIN_LEVEL": 100,  # Минимальная температура (умножено на 10) Диапазон: -400...600 ºС, По умолчанию 100
    "HUMID_MAX_LEVEL": 800,  # Максимальная относительная влажность (умножено на 10) Диапазон 100...900 %, По умолчанию 800
    "HUMID_HYST": 300,  # Гистерезис (умножено на 10) влажности при осушении, 10...800%, По умолчанию 300
    "HUMID_ALARM_LEVEL": 900,  # Порог аварийной влажности (умножено на 10) 60...990 %, По умолчанию 900
    "SENSOR_PRIO1": 2,  # Приоритет для датчика температуры Диапозон 0...2 , Датчик с более высоким приоритетом является основным
    "SENSOR_PRIO2": 1,  # Приоритет для датчика температуры Диапозон 0...2 , Датчик с более высоким приоритетом является основным
    "SENSOR_PRIO3": 0,  # Приоритет для датчика температуры Диапозон 0...2 , Датчик с более высоким приоритетом является основным
    "SENSOR_EXT": 255,  # Наружный датчик температуры 0 - Тц1, 1 - Тц2, 255 - наружный датчик не выбран
    "SENSORS_ENABLED": 15,  # Включить датчики Битовая маска: Бит 0 - Tц1 Бит 1 - Tц2 Бит 2 - Tц3 Бит 3 - Ta, По умолчанию 15(включены все)
    "SEARCH_FOR_SENSORS": 1,  # Поиск датчиков Тц1 и Тц2 (1 - искать новые 1-wire датчики)
    "RELAY_MIN_SW_TIME": 30,  # Минимальное время переключения реле Диапазон 0...600 сек, По умолчанию 30
    "OTP_ENABLED": 0,  # Отключение АО при перегреве Диапазон 0...1, По умолчанию 0
    "OTP_LEVEL": 500,  # Температура перегрева х10 (отключения АО), 0...600ºС, По умолчанию 50
    "OTP_HYST": 50,  # Гистерезис температуры включения АО х10, 10...100 ºС, По умолчанию 50
    ...
    "FAN_HYST": 50,  # Гистерезис температуры включения вентилятора х10, 10...100 ºС, По умолчанию 50
    "HEAT_HYST": 50,  # Гистерезис температуры включения нагрева х10, 10...100 ºС, По умолчанию 50
    "MODBUS_ADDR": 1,  # ModBus-адрес устр-ва Диапазон 1...247, По умолчанию 1
}
...
ATTEMPTS_TO_MODBUS_CONNECTIONS: typing.Final[int] = (
    3  # Количество попыток подключения по Modbus протоколу (рекомендовано 3)
)
WRITE_MODBUS_TIMEOUT: typing.Final[float] = (
    0.2  # Таймаут между запись значений (рекомендовано 0.2)
)
LOG_FILE_NAME_WRITE: typing.Final[str] = (
    "../write_registers_log.json"  # Название файла для логов записи
)
LOG_FILE_NAME_READ: typing.Final[str] = (
    "../read_registers_log.json"  # Навзвание фала для логов чтения
)
